...
const ThanhToan = () => {
  ...
  const handleComplete = async () => {
    // 1. Kiểm tra kỹ dữ liệu trước khi đóng gói
    const sc_id = state.suat_chieu_id || state.showtime?.id; // Lấy từ 1 trong 2 nguồn

    const orderData = {
      user_id: Number(user.id), // Ép kiểu số
      suat_chieu_id: Number(sc_id), // Ép kiểu số
      seats: Array.isArray(state.seats) ? state.seats : [], 
      total_price: Number(state.total) || 0,
      movie_info: {
        ten: String(state.movie?.ten || "Phim"),
        ngay: String(timeData.ngay), // "DD/MM/YYYY"
        gio: String(timeData.gio)    // "HH:mm"
      }
    };

    // 2. Chặn gửi nếu thiếu ID quan trọng
    if (!orderData.suat_chieu_id || isNaN(orderData.suat_chieu_id)) {
      alert("Lỗi: ID suất chiếu không hợp lệ!");
      console.error("Dữ liệu state hiện tại:", state);
      return;
    }
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        alert("Lỗi hệ thống: " + (result.error || "Không xác định"));
      }
    } catch (error) {
      alert("Lỗi kết nối máy chủ! Vui lòng kiểm tra Server Node.js");
    }
  };
